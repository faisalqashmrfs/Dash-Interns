...
export default function Onecontract({ nAVbAR, setnAVbAR }) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`https://test.black-analysis-solutions.com/api/contract/${id}`,
          {
            headers: {
  ...
  const handleDelete = () => {
    setMasseg(false);
    axios.delete(`https://test.black-analysis-solutions.com/api/contract/${id}`,
      {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        ...
        <section>
          ...
          <section className={nAVbAR ? 'DataIntern-off' : 'DataIntern'}>
            ...
            <div className='Controle-Certificate'>
              <button onClick={() => createPDF(Data.full_name)}>طباعة العقد</button>
          ...
          <div className='certificate1 certificate1x'>
            <div className='box-certificate'>
              ...
              <div id="pdf-content-page2" className='PDF-containarx2'>
                ...
                <p>
                  أوافق على الشروط والأحكام وموافقتي وكتابة اسمي بالكامل يعتبر توقيع على العقد والتزامي بشروط التدريب
                </p>
